    ...
    <div class="col-12">
        <div class="card mb-4">
            <div class="card-header pb-0 d-flex justify-content-between align-items-center">
                ...
                <a href="{{ route('dashboard.testimonials.index') }}" class="btn btn-secondary btn-sm ms-auto">Kembali</a>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" value="{{ $testimonial->product->name ?? 'Produk Dihapus' }}" readonly>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" value="{{ $testimonial->created_at->format('d M Y, H:i') }}" readonly>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <div class="form-control bg-light">
                                <span class="badge badge-sm bg-gradient-{{ $testimonial->is_displayed ? 'success' : 'secondary' }}">
                    ...
                    <div class="col-md-12 mb-4">
                        <label class="form-control-label d-block">Foto Testimonial</label>
                        <div class="d-flex flex-wrap gap-2">
                            @if($testimonial->image)
                                <img src="{{ asset('storage/' . $testimonial->image) }}" class="rounded shadow-sm" style="max-width: 300px; max-height: 300px; object-fit: contain;">
                            @else
                                <div class="text-xs text-secondary italic">Tidak ada foto testimonial.</div>
                            @endif
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-4 d-flex justify-content-between">
                    <form action="{{ route('dashboard.testimonials.destroy', $testimonial->id) }}" method="POST" class="delete-form-{{ $testimonial->id }}">
                        @csrf
                        @method('DELETE')
                        <button type="button" class="btn btn-outline-danger btn-sm delete-btn" data-id="{{ $testimonial->id }}">
                    ...
                    <div class="d-flex gap-2">
                        <form action="{{ route('dashboard.testimonials.toggle', $testimonial->id) }}" method="POST">
                            @csrf
                            <button type="submit" class="btn btn-{{ $testimonial->is_displayed ? 'secondary' : 'success' }} btn-sm font-weight-bold">
