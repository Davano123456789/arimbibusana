        ...
        <div class="card mb-4">
            <div class="card-header pb-0 d-flex flex-wrap justify-content-between align-items-center gap-2">
                ...
                <div class="d-flex align-items-center ms-md-auto gap-2 flex-wrap">
                    <form action="{{ route('dashboard.products.index') }}" method="GET" class="mb-0">
                        <div class="input-group input-group-sm">
                            <input type="text" name="search" class="form-control" placeholder="Cari nama produk..." value="{{ request('search') }}">
                            <button class="btn btn-outline-primary mb-0" type="submit">Cari</button>
                            @if(request('search'))
                                <a href="{{ route('dashboard.products.index') }}" class="btn btn-outline-secondary mb-0">Reset</a>
                            @endif
                        </div>
                    </form>
                    <a href="{{ route('dashboard.products.create') }}" class="btn btn-primary btn-sm mb-0">Tambah Produk</a>
                </div>
            </div>
