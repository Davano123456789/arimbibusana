...
<head>
    ...
    <title>Login — Arimbi Queen</title>
    <meta name="description" content="Login ke Arimbi Queen untuk mulai belanja busana muslimah premium.">
...
<body class="min-h-screen bg-cream flex items-center justify-center p-4">
    ...
    <div class="w-full max-w-md bg-white rounded-[2.5rem] shadow-xl shadow-gray-200/60 p-10 slide-up">
        ...
        <div class="text-center mb-9">
            ...
            <h1 class="mt-5 text-2xl font-playfair font-bold text-gray-900 leading-snug">
                Masuk ke Akun<br>
                <span class="text-accent">Anda</span>
            </h1>
        </div>
        ...
        @if(session('error'))
        <div class="mb-5 p-3.5 bg-red-50 border border-red-100 rounded-xl">
            <p class="text-xs text-red-600 flex items-center gap-1.5"><i class="fa-solid fa-circle-exclamation text-red-400"></i> {{ session('error') }}</p>
        </div>
        @endif
        ...
        <form action="{{ url('/login') }}" method="POST" class="space-y-4" novalidate>
            ...
            <div>
                <div class="flex items-center justify-between mb-1.5">
                    <label for="password" class="text-[11px] font-semibold tracking-widest uppercase text-gray-400 ml-0.5">Password</label>
                    <a href="#" class="text-[11px] text-accent font-semibold hover:underline">Lupa password?</a>
                </div>
                <div class="relative">
                    <span class="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-300 text-sm pointer-events-none">
            ...
            <div class="flex items-center gap-2 pt-1">
                <input type="checkbox" id="remember" name="remember"
                    class="w-4 h-4 rounded border-gray-300 text-accent cursor-pointer focus:ring-accent/20">
                <label for="remember" class="text-xs text-gray-500 cursor-pointer select-none">Ingat saya selama 30 hari</label>
            </div>

            <!-- Submit -->
            <button type="submit"
                class="w-full mt-2 bg-accent text-white text-sm font-semibold py-3.5 rounded-xl shadow-lg shadow-accent/20 hover:bg-accent-light active:scale-[0.98] transition-all flex items-center justify-center gap-2">
                Masuk
        ...
        <p class="text-center text-xs text-gray-400">
            Belum punya akun?
            <a href="{{ url('/register') }}" class="text-accent font-semibold hover:underline ml-1">Daftar sekarang</a>
        </p>
    ...
    <div class="fixed inset-0 -z-10 overflow-hidden pointer-events-none">
        <div class="absolute top-1/4 -left-32 w-72 h-72 bg-accent/5 rounded-full blur-3xl"></div>
        <div class="absolute bottom-1/4 -right-32 w-72 h-72 bg-accent/5 rounded-full blur-3xl"></div>
    </div>
</body>
</html>
